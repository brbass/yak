...
Flux_Iterator(Power_Iteration const &pi,
    ...
    Vector_Operator(pi.phi_size() + pi.number_of_augments(),
                    pi.phi_size() + pi.number_of_augments()),
...
apply(vector<double> &x) const
{
    ...
    shared_ptr<Sweep_Operator> Linv =
        dynamic_pointer_cast<Sweep_Operator>(pi_.sweeper_);
    Assert(Linv);
    ...
    if (pi_.preconditioned_)
    {
        shared_ptr<Preconditioner> Cinv
            = dynamic_pointer_cast<Preconditioner>(pi_.preconditioner_);
        Assert(Cinv);
    ...
    shared_ptr<Vector_Operator> I
        = make_shared<Identity_Operator>(pi_.phi_size() + pi_.number_of_augments());
