...
# shape_values = np.around(np.arange(0.10, 0.3, 0.01), decimals=2)
shape_values = np.around(np.arange(0.2, 3.1, 0.1), decimals=1)
# distance_values = np.around(np.arange(0.05, 0.15, 0.05), decimals=2)
distance_values = [0.2]
ord_values = [1]
input_filenames = [io_folder + "pin_{1}_{0}_{2}.xml".format(i, j, k) for i in shape_values for j in distance_values for k in ord_values]
output_filenames = [io_folder + "pin_{1}_{0}_{2}.xml.out".format(i, j, k) for i in shape_values for j in distance_values for k in ord_values]
output_values = [(i, j, k) for i in shape_values for j in distance_values for k in ord_values]
...
for i, shape in enumerate(shape_values):
    for j, distance in enumerate(distance_values):
        for k, ord in enumerate(ord_values):
            input_string = template_string.replace("(SHAPE)", str(shape)).replace("(DISTANCE)", str(distance)).replace("(ORD)", str(ord))
            input_file = open(input_filenames[k + len(ord_values) * (j + len(distance_values) * i)], "w")
...
def run_problem(index):
    test_description = "\tInd: {2} / {3}\tDist: {0}\tShape: {1}\tOrd: {4}".format(output_values[index][1], output_values[index][0], index + 1, number_of_cases, output_values[index][2])
...
def init_results_file():
    result_string = "shape\tdist\tord\tk_eff\tkryl_iter\ttime\n"
